...
export default function TutorApp({ role = "owner", onLogout }: { role?: "owner" | "teacher" | "student"; onLogout?: () => void }) {
  ...
  const applyData = useCallback((data: AppData) => {
    setLessons(data.lessons); setStudents(data.students); setRequests(data.requests);
  ...
  const addPayment = async (studentId: string, count: number) => {
    try { await saveAppData({ action: "addPayment", studentId, count }); await reload(); toast.success(`Добавлено ${count} занятий`); }
...
function History({ payments = false }: { payments?: boolean }) { const rows = payments ? [["3 сентября 2026", "+8", "Оплата абонемента"], ["18 августа 2026", "+4", "Оплата занятий"]] : [["17 сентября 2026", "−1", "Урок проведён"], ["10 сентября 2026", "−1", "Урок проведён"], ["3 сентября 2026", "+8", "Оплата абонемента"]]; return <div className="card divide-y">{rows.map((row) => <div key={row[0]} className="flex items-center gap-4 p-5"><span className={`grid size-11 place-items-center rounded-xl font-bold ${row[1].startsWith("+") ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}>{row[1]}</span><span><strong className="block">{row[2]}</strong><span className="text-sm text-slate-500">{row[0]}</span></span></div>)}</div>; }
...
function PaymentDialog({ student, onPay }: { student: Student; onPay: (id: string, count: number) => void }) {
  const [count, setCount] = useState(8); return <Dialog><DialogTrigger asChild><Button size="lg" className="h-11 rounded-xl bg-indigo-600"><Plus />Добавить оплату</Button></DialogTrigger><DialogContent className="rounded-3xl sm:max-w-lg"><DialogHeader><DialogTitle className="text-2xl">Добавить оплату</DialogTitle><DialogDescription>{student.name}</DialogDescription></DialogHeader><div className="rounded-2xl bg-rose-50 p-4 font-semibold text-rose-700">Текущий баланс: {student.balance} занятия</div><Field label="Количество занятий"><Input aria-label="Количество занятий" type="number" min="1" value={count} onChange={(event) => setCount(Number(event.target.value))} className="h-11 rounded-xl" /></Field><Field label="Дата оплаты"><Input aria-label="Дата оплаты" type="date" defaultValue="2026-09-18" className="h-11 rounded-xl" /></Field><div className="rounded-2xl bg-emerald-50 p-4 text-emerald-800">После оплаты баланс составит <strong>{student.balance + count} занятий</strong></div><DialogFooter><DialogClose asChild><Button variant="outline">Отмена</Button></DialogClose><DialogClose asChild><Button onClick={() => onPay(student.id, count)} className="bg-indigo-600">Добавить оплату</Button></DialogClose></DialogFooter></DialogContent></Dialog>;
}
