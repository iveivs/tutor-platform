...
import {
  ...
  Home, LogOut, Plus, Search, Send, Settings, Trash2, UserRound, UsersRound, WalletCards,
} from "lucide-react";
...
export default function TutorApp({ role = "owner", user, onLogout }: { role?: "owner" | "teacher" | "student"; user?: CurrentUser; onLogout?: () => void }) {
  ...
  const applyData = useCallback((data: AppData) => {
    setLessons(data.lessons); setStudents(data.students); setRequests(data.requests); setBalanceEntries(data.balanceEntries ?? []); setNotifications(data.notifications ?? []); setTeacherName(data.teacherName ?? "Преподаватель"); if (data.profile) setProfile(data.profile);
  ...
  const addStudent = async (student: Omit<Student, "id">) => {
    try {
      await saveAppData({ action: "createStudent", name: student.name, email: student.email, floating: student.floating });
      await reload();
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="min-h-screen pb-24 lg:ml-[272px] lg:pb-0">
        ...
        {view === "today" && <TodayView lessons={lessons} students={students} requests={requests} onAdd={addLesson} setView={setView} onOpenStudent={(name) => { const student = students.find((item) => item.name === name); if (student) { setSelectedStudent(student.id); setView("student"); } }} />}
...
function TodayView({ lessons, students, requests, onAdd, setView, onOpenStudent }: { lessons: Lesson[]; students: Student[]; requests: LessonRequest[]; onAdd: (lesson: LessonDraft) => void; setView: (view: View) => void; onOpenStudent: (name: string) => void }) {
...
function StudentsView({ students, onAdd, onOpen }: { students: Student[]; onAdd: (student: Omit<Student, "id">) => Promise<boolean>; onOpen: (id: string) => void }) {
...
function StudentRequestDialog({ type, lesson, student, onRequest, fullWidth = false }: { type: "cancel" | "reschedule" | "new_lesson"; lesson?: Lesson; student: Student; onRequest: (body: { requestType: "cancel" | "reschedule" | "new_lesson"; lessonId?: string; proposedDate?: string; proposedTime?: string; message?: string; studentId?: string }) => void; fullWidth?: boolean }) {
  const [open, setOpen] = useState(false); const [date, setDate] = useState(lesson?.date ?? shiftDate(today(), 1)); const [time, setTime] = useState(lesson?.time ?? "17:00"); const [message, setMessage] = useState("");
  const label = type === "cancel" ? "Запросить отмену" : type === "reschedule" ? "Запросить перенос" : "Предложить время урока";
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button size="lg" variant={type === "cancel" ? "outline" : "default"} className={`${fullWidth ? "w-full" : ""} h-12 rounded-xl ${type === "cancel" ? "" : "bg-indigo-600"}`}>{label}</Button></DialogTrigger><DialogContent className="rounded-3xl sm:max-w-lg"><DialogHeader><DialogTitle>{label}</DialogTitle><DialogDescription>{type === "cancel" ? "Преподаватель получит запрос. Своевременный запрос не будет списан, даже если останется без ответа." : "Предложите удобные дату и время. Урок появится в расписании после подтверждения преподавателем."}</DialogDescription></DialogHeader>{type !== "cancel" && <div className="grid gap-4 sm:grid-cols-2"><Field label="Дата"><Input type="date" value={date} min={today()} onChange={(event) => setDate(event.target.value)} /></Field><Field label="Время"><Input type="time" value={time} onChange={(event) => setTime(event.target.value)} /></Field></div>}<Field label="Комментарий"><Input value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Необязательно" /></Field><DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>Назад</Button><Button className="bg-indigo-600" onClick={() => { onRequest({ requestType: type, lessonId: lesson?.id, proposedDate: type === "cancel" ? undefined : date, proposedTime: type === "cancel" ? undefined : time, message, studentId: student.id }); setOpen(false); }}>Отправить запрос</Button></DialogFooter></DialogContent></Dialog>;
}
